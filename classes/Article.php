...
/* Un UPDATE préparé
$update = $dbh->prepare('UPDATE `utilisateurs` SET `verrouille` = :verrouille WHERE `login` = :login');
...
// Autres méthodes
	/*public function ajouter()
	{
	    ...
	    $q = "INSERT INTO article(id,titre,image,text,d_ajout) 
	    	  VALUES (null, :titre, :titre,:image,:text, NOW())";
		$stmt = $this->get_cnx()->prepare($q);

		$stmt->bindParam(':titre', $this->_titre);
		$stmt->bindParam(':sujet', $this->_text);
		$stmt->bindParam(':image', $this->_image);
